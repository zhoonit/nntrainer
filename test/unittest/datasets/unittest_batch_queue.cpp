...
TEST(BatchQueue, pushTimeOut_n) {
  nntrainer::BatchQueue bq(1);
  bq.wait_and_push(data(1));
  EXPECT_THROW(bq.wait_and_push(data(2)), std::runtime_error);
  auto result = bq.wait_and_pop();
  test_data(*result, 1);
}

TEST(BatchQueue, popTimeOut_n) {
  nntrainer::BatchQueue bq(3);
  EXPECT_THROW(bq.wait_and_pop(), std::runtime_error);
}
