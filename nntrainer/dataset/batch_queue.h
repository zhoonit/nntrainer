...
public:
  ...
  /**
   * @brief push data to queue, if the batch queue is full, wait for a bounded
   * time
   *
  ...
  /**
   * @brief pop data from the queue, wait for a bounded time if empty
   *
...
private:
  ...
  std::condition_variable q_reader_cv;
  std::condition_variable q_writer_cv;
